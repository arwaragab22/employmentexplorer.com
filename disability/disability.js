...
<li style="display: flex;align-items: flex-start;gap:3px"><span>4</span>
  <div>  
  لِإِعَادَة سَمَاعِ اَلْإرْشَادَاتَ مَرَة أُخْرَى اِضْغَطْ عَلَى زِرُّ اَلتَّبْدِيلِ

  + 1  </div>
  </li>
  <li style="display: flex;align-items: flex-start;gap:3px"><span>5</span><div>    
    لِإِعَادَة قِرَاءَةِ اَلصَّفْحَةِ مِنْ اَلْبِدَايَةِ اِضْغَطْ عَلَى زِرُّ اَلتَّبْدِيلِ
...
<li style="display: flex;align-items: flex-start;gap:3px"><span>4</span>
  ...
  <li style="display: flex;align-items: flex-start;gap:3px"><span>5</span><div>    
    لِإِعَادَة قِرَاءَةِ اَلصَّفْحَةِ مِنْ اَلْبِدَايَةِ اِضْغَطْ عَلَى alt + 2  </div></li>
    <li style="display: flex;align-items: flex-start;gap:3px"><span>6  </span><div>  
...
if(document.getElementById("disabilitymodal").style.dispaly!="block"){
  // document.getElementById("disabilitymodal").style.zIndex="-1"
    ...
    if( true === event.altKey && '0'=== event.key ) {
      if (utterance) {
        speechSynthesis.cancel();
    ...
    if( true === event.altKey && '1' === event.key ) {
      if (utterance) {
        speechSynthesis.cancel();
      }
      document.getElementById("disabilitymodal").style.opacity="1"
    ...
    if(    document.getElementsByClassName("closemodel")[0]){
      ...
      if(window.location.href.includes("home")){
        ...
        // document.getElementById("askshowagain").style.display="none";
        document.getElementById("readagain").style.display="none";
        if(        document.getElementById("readagain1")
        ){
          document.getElementById("readagain1").style.display="none";

      }
      if(        document.getElementById("readagain2")
      ){
        document.getElementById("readagain2").style.display="none";
...
if(   document.getElementById('blind')){
  document.getElementById('blind').addEventListener("click",function(){
   ...
   /*if user want to active  Screen reader feature  click alt + b*/
   document.addEventListener( 'keydown', (event) => {
   ...
   if( true === event.altKey && '1' === event.key ) {
    ...
    document.getElementsByClassName("quesblind3")[0],focus();
     ...
     /*if user want to read  first element in page  click alt + 1*/
   if (true === event.altKey && '2' === event.key ) {
...
document.getElementById("Label1country").addEventListener('keydown', function(event) {
  ...
  if (event.shiftKey && event.key === 'Tab') {
    event.preventDefault();
    setTimeout(()=>{
      txt("عُنْوَانُ اَلصَّفْحَةِ بَوَّابَةَ اَلْوَظَائِفِ اَلْحُكُومِيَّةِ ");
    },100)
...
txt("انت الان على عنصر زر للضغط اضغط enter");
    ...
    const allelementtabindex = document.querySelectorAll('[tabindex]');
    console.log(allelementtabindex[allelementtabindex.length-1]);
    allelementtabindex[allelementtabindex.length-1].addEventListener('keydown', function(event) {

  // Check if the Shift key and the Tab key were pressed simultaneously
  if ( event.key === 'Tab'&&!event.shiftKey) {
    event.preventDefault();
    setTimeout(()=>{
      txt(" هَذَا آخِرٌ عُنْصُرٍ فِي اَلصَّفْحَةِ لِإِعَادَةِ قِرَاءَةِ اَلصَّفْحَةِ مِنْ اَلْبِدَايَةِ اِضْغَطْ عَلَى زِرُّ اَلتَّبْدِيلِ + 2"
